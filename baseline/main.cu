...
Result exec(int block_size, int amount_blocks) {
    ...
    scan_lookback<<<amount_blocks, block_size, block_size * sizeof(float) * 2>>>(input, partition_descriptiors);
	...
	std::cerr << "[!] Event Sync failed" << cudaGetErrorName(error) << std::endl;
	cudaFree(input);
	cudaFree(partition_descriptiors);
	free(gold);
	exit(EXIT_FAILURE);
	...
	for (int i = 0; i < amount_blocks; i++) {
	    printf("State %d got state %d and inclusive_prefix %f and agg %f\n", i, partition_descriptiors[i].flag, partition_descriptiors[i].inclusive_prefix, partition_descriptiors[i].aggregate);
	}
